...
import { PYTHON_SOURCE_CODE, REQUIREMENTS_TXT, SETUP_SHELL_SCRIPT } from '../data/pythonScript';
...
export const Step4PythonScriptHub: React.FC = () => {
  const [activeFile, setActiveFile] = useState<'qwen3' | 'script' | 'setup_qwen3' | 'requirements' | 'setup'>('qwen3');
  ...
  const getActiveContent = () => {
    switch (activeFile) {
      ...
      case 'setup_qwen3':
        return QWEN3_TTS_COMMAND_SCRIPT;
      case 'requirements':
  ...
  const getFileName = () => {
    switch (activeFile) {
      ...
      case 'setup_qwen3':
        return 'setup_qwen3_tts.command';
      case 'requirements':
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-slate-950 border-b border-slate-800 overflow-x-auto">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>qwen3_tts_dubber.py (0% Robot)</span>
            </button>
            ...
            <button
              id="tab-setup-qwen3-btn"
              onClick={() => setActiveFile('setup_qwen3')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono font-medium transition-colors whitespace-nowrap cursor-pointer ${
                activeFile === 'setup_qwen3'
                  ? 'bg-slate-800 text-cyan-400 border border-slate-700'
                  : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              <Terminal className="w-3.5 h-3.5 text-amber-400" />
              <span>setup_qwen3_tts.command</span>
            </button>
